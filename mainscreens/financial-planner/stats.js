...
const GoalsStats = ({balance}) => {
    ...
    const percentage = hasGoal ? (balance / goal) * 100 : 0;
    return (<>
   ...
   {hasGoal ? (
        diff > 0 ? (
          <Text style={{ alignSelf: 'center', fontFamily: 'LatoBold', fontSize: 18 }}>
            You are ${goal - balance} away from your monthly goal!
          </Text>
        ) : (
          <WarningMessage />
        )
        ...
        <Text style={{ alignSelf: 'center', fontFamily: 'LatoBold', fontSize: 18 }}>
          You do not have a monthly goal right now! Set it up now!
        </Text>
    )}
    <Spacer h='1%'/>
